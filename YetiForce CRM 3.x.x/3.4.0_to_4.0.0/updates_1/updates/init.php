	...
	private function getConfigurations()
	{
		return [
				['name' => 'config/config.inc.php', 'conditions' => [
						...
						['type' => 'update', 'search' => "dbconfig['db_type']", 'replace' => ['mysqli', 'mysql']],
				],
			],
...
"],
						['type' => 'add', 'search' => "messages_cache", 'checkInContents' => "config['reply_mode']", 'addingType' => 'after', 'value' => "\$config['reply_mode'] = 1;
// Debug
"],
				]
			],
...
"],
				...
				['name' => 'config/modules/Mail.php', 'conditions' => [
						['type' => 'remove', 'search' => 'h - Horinzontal, v - vertical'],
						['type' => 'remove', 'search' => 'ORIENTATION_PANEL_VIEW'],
						['type' => 'add', 'search' => "];", 'checkInContents' => 'MAILER_REQUIRED_ACCEPTATION_BEFORE_SENDING', 'addingType' => 'before', 'value' => "	// Required acceptation before sending mails
		...
		switch ($index) {
			case 1:
				$fields = [
						...
						['type' => ['change', 'Type'], 'validType' => 'tinyint', 'name' => 'email_template_priority', 'table' => 'vtiger_email_template_priority', 'sql' => "ALTER TABLE `vtiger_email_template_priority` 
							CHANGE `email_template_priority` `email_template_priority` tinyint(1)   NULL after `email_template_priorityid` , 
							CHANGE `presence` `presence` tinyint(1)   NULL DEFAULT 1 after `email_template_priority` , 
							CHANGE `sortorderid` `sortorderid` smallint(6)   NULL DEFAULT 0 after `presence` ;"],
						['type' => ['change', 'Type'], 'validType' => 'varchar(255', 'name' => 'email_template_type', 'table' => 'vtiger_email_template_type', 'sql' => "ALTER TABLE `vtiger_email_template_type` 
							CHANGE `email_template_type` `email_template_type` varchar(255) NULL after `email_template_typeid` , 
							CHANGE `presence` `presence` tinyint(1)   NULL DEFAULT 1 after `email_template_type` , 
							CHANGE `sortorderid` `sortorderid` smallint(6)   NULL DEFAULT 0 after `presence` ;"],
				];
				break;
			case 2:
...
//							ADD CONSTRAINT `fk_1_notification` FOREIGN KEY (`notificationid`) REFERENCES `vtiger_crmentity` (`crmid`) ON DELETE CASCADE ;"],
	...
	private function actionMapp($actions)
	{
		...
		foreach ($actions as $action) {
			...
			if ($action['type'] === 'remove') {
				if ($key = $db->getSingleValue($result)) {
					$db->delete('vtiger_actionmapping', 'actionid = ?', [$key]);
					$db->delete('vtiger_profile2utility', 'activityid = ?', [$key]);
				}
				continue;
			}
			if (!$db->getRowCount($result)) {
	...
	private function updatePack()
	{
		...
		$db->update('vtiger_cvcolumnlist', ['columnname' => 'u_yf_notification:title:title:Notification_FL_TITLE:V'], 'columnname = ?', ['vtiger_notification:title:title:Notification_FL_TITLE:V']);
		$db->update('vtiger_cvcolumnlist', ['columnname' => 'u_yf_notification:number:number:Notification_FL_NUMBER:V'], 'columnname = ?', ['vtiger_notification:number:number:Notification_FL_NUMBER:V']);
		...
		$db->update('vtiger_field', ['defaultvalue' => 'PLL_UNREAD'], 'tablename = ? AND columnname = ?', ['u_yf_notification', 'notification_status']);
		$db->update('vtiger_field', ['defaultvalue' => 'PLL_USERS'], 'tablename = ? AND columnname = ?', ['u_yf_notification', 'notification_type']);
		...
		$db->update('vtiger_dataaccess_cnd', ['val' => 'PLL_LEAD_ACQUIRED'], 'val = ? AND fieldname = ?', ['LBL_LEAD_ACQUIRED', 'leadstatus']);
		$db->update('vtiger_dataaccess_cnd', ['val' => 'PLL_LEAD_UNTAPPED'], 'val = ? AND fieldname = ?', ['LBL_LEAD_UNTAPPED', 'leadstatus']);
		$db->update('vtiger_dataaccess_cnd', ['val' => 'PLL_TO_REALIZE'], 'val = ? AND fieldname = ?', ['LBL_TO_REALIZE', 'leadstatus']);
		$db->update('vtiger_dataaccess_cnd', ['val' => 'PLL_CONTACTS_IN_THE_FUTURE'], 'val = ? AND fieldname = ?', ['LBL_CONTACTS_IN_THE_FUTURE', 'leadstatus']);
		...
		if (!$db->getRowCount($resultC)) {
			$data = $db->insert('u_yf_dashboard_type', [
				'name' => 'LBL_MAIN_PAGE', 'system' => 1
			]);
			$db->update('vtiger_module_dashboard_blocks', ['dashboard_id' => $data['id']]);
			$db->update('vtiger_module_dashboard_widgets', ['dashboardid' => $data['id']]);
		}
		...
		$resultC = $db->pquery('SELECT fieldid FROM vtiger_field WHERE tabid = ? AND columnname in (?,?,?);', [$tabId, 'is_admin', 'roleid', 'status']);
		$fieldTypeIds = $db->getArrayColumn($resultC);
		if ($fieldTypeIds) {
			$resultP = $db->query('SELECT profileid FROM vtiger_profile;');
		...
		$db->delete('vtiger_cvcolumnlist', '`columnname` IN (' . $db->generateQuestionMarks($data) . ')', $data);
		...
		$db->update('vtiger_ws_fieldtype', ['fieldtype' => 'userCreator'], '`uitype` = ?', [52]);
		...
		$tabId = \vtlib\Functions::getModuleId('Notification');
		$db->update('vtiger_entityname', ['entityidfield' => 'notificationid', 'entityidcolumn' => 'notificationid'], 'entityidfield = ? and tabid = ?', ['id', $tabId]);
	...
	private function updateFileExtensionInDB()
	{
		...
		$query .= ' ELSE `handler_file` END WHERE `handler_file` IN (' . $db->generateQuestionMarks($cron) . ')';
...
//			'LogoutHandler' => '',
		...
		$relationsCombine = [
			...
			'OSSTimeControl_TimeControl_Handler' => ['include_modules' => 'OSSTimeControl', 'owner_id' => \vtlib\Functions::getModuleId('OSSTimeControl')],
		...
		$query .= $set . ' ELSE `include_modules` END '
			. " , priority = CASE "
			. " WHEN `event_name`='EntityAfterSave' AND handler_class NOT IN('ModTracker_ModTrackerHandler_Handler','Vtiger_Workflow_Handler','Vtiger_RecordLabelUpdater_Handler','PBXManager_PBXManagerHandler_Handler','ServiceContracts_ServiceContractsHandler_Handler', 'Accounts_SaveChanges_Handler') THEN 3 "
		...
		$db->delete('vtiger_eventhandlers', 'handler_class = ?', ['PBXManagerBatchHandler']);
		\App\Log::trace('Exiting ' . __METHOD__);
	}
	...
	{
		\App\Log::trace('Entering ' . __METHOD__ . ": $userId");

		...
		foreach ($modules as $moduleName) {
			...
			if ($moduleBaseInstance) {
				...
				if ($moduleBaseInstance->isentitytype) {
					if ($moduleBaseInstance->name !== 'Emails') {
						$deletedRecords = (new \App\Db\Query)->select(['vtiger_crmentity.crmid'])->from('vtiger_crmentity')->innerJoin($focus->table_name, "{$focus->table_name}.{$focus->table_index} = vtiger_crmentity.crmid")->where(['vtiger_crmentity.deleted' => 1])->column();
						if ($deletedRecords) {
							$this->removeRecordsModule($deletedRecords);
						}
					}
		...
		foreach ($modules as $module) {
			if (file_exists('cache/updates/' . $module . '.xml') && !\vtlib\Module::getInstance($module)) {
				...
				foreach ($locations as $loc) {
					...
					$functions->recurseCopy($dirName . $loc, $loc, true);
					$functions->recurseDelete($dirName . $loc);
				}
...
"],
	...
	public function postupdate()
	{
		...
		\App\Log::trace('refreshMenuFiles start');
		$menuRecordModel = new Settings_Menu_Record_Model();
		$menuRecordModel->refreshMenuFiles();
		\vtlib\Deprecated::createModuleMetaFile();
		\vtlib\Access::syncSharingAccess();
		\App\Log::trace('syncSharingAccess end');

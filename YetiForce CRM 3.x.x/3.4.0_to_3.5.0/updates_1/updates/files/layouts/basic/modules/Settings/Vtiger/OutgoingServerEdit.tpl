				...
				<table class="table table-bordered table-condensed themeTableColor">
					...
					<tbody>
						...
						<tr><td class="{$WIDTHTYPE}"><label class="muted pull-right marginRight10px">{vtranslate('LBL_FROM_EMAIL', $QUALIFIED_MODULE)}</label></td>
							<td class="{$WIDTHTYPE}" style="border-left: none;"><input class="form-control" type="text" name="from_email_field" data-validation-engine="validate[funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-validator={\App\Json::encode([['name' => 'Email']])} value="{$MODEL->get('from_email_field')}" /></td></tr>

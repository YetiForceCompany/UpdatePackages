 ...
 *************************************************************************************/"use strict";var Vtiger_Index_Js={showLocation:function showLocation(element){app.showModalWindow(null,"index.php?module=OpenStreetMap&view=MapModal",function(container){var mapView=new OpenStreetMap_Map_Js;mapView.registerModalView(container),container.find(".searchValue").val($(element).data("location")),container.find(".searchBtn").trigger("click");});},massAddDocuments:function massAddDocuments(url){app.showModalWindow(null,url,function(container){var uploadButton=container.find("#filesToUpload"),template=container.find(".fileContainer"),uploadContainer=container.find(".uploadFileContainer"),form=container.find("form");uploadButton.on("change",function(){uploadContainer.find(".fileItem").remove();for(var files=uploadButton[0].files,i=0;i<files.length;i++)uploadContainer.append(template.html()),uploadContainer.find("[name=\"nameFile[]\"]:last").val(files[i].name);}),form.on("submit",function(e){e.preventDefault(),app.removeEmptyFilesInput(form[0]);var formData=new FormData(form[0]);url="index.php","Detail"===app.getViewName()&&(formData.append("createmode","link"),formData.append("return_module",app.getModuleName()),formData.append("return_id",app.getRecordId()));var params={url:url,type:"POST",data:formData,processData:!1,contentType:!1},progressIndicatorElement=$.progressIndicator({blockInfo:{enabled:!0}});AppConnector.request(params).done(function(){progressIndicatorElement.progressIndicator({mode:"hide"}),app.hideModalWindow();if("Detail"===app.getViewName()){var detailView=Vtiger_Detail_Js.getInstance(),selectedTabElement=detailView.getSelectedTab();if(selectedTabElement.data("reference")==="Documents")detailView.reloadTabContent();else if(detailView.getContentHolder().find(".detailViewBlockLink").data("reference")==="Documents")Vtiger_RelatedList_Js.getInstance(detailView.getRecordId(),app.getModuleName(),selectedTabElement,"Documents").loadRelatedList();else {var updatesWidget=detailView.getContentHolder().find("[data-type='RelatedModule'][data-name='Documents']");if(0<updatesWidget.length){var _params=detailView.getFiltersData(updatesWidget);detailView.loadWidget(updatesWidget,_params.params);}}}else Vtiger_List_Js.getInstance().getListViewRecords();});});});},getEmailFromRecord:function getEmailFromRecord(record,module,maxEmails){var aDeferred=$.Deferred(),progress=$.progressIndicator({position:"html",blockInfo:{enabled:!0}});return AppConnector.request({dataType:"html",data:{module:"OSSMail",action:"GetMail",sourceModule:module,sourceRecord:record,maxEmails:maxEmails}}).done(function(data){progress.progressIndicator({mode:"hide"}),"{"==data.substring(0,1)?(data=JSON.parse(data),data=data.result,aDeferred.resolve(data)):app.showModalWindow(data,function(data){data.find(".selectButton").on("click",function(){if(data.find("input:checked").length){var email=data.find("input:checked").val();app.hideModalWindow(),aDeferred.resolve(email);}else app.showNotify({text:app.vtranslate("JS_SELECT_AN_OPTION"),type:"info"});});});}).fail(function(error){progress.progressIndicator({mode:"hide"}),aDeferred.reject(error);}),aDeferred.promise()},registerMailButtons:function registerMailButtons(container){var thisInstance=this;container.find(".sendMailBtn:not(.mailBtnActive)").each(function(){var sendButton=$(this);sendButton.addClass("mailBtnActive"),sendButton.on("click",function(e){e.stopPropagation();var url=sendButton.data("url"),popup=sendButton.data("popup"),toMail=sendButton.data("to");toMail&&(url+="&to="+encodeURIComponent(toMail)),app.getRecordId()&&sendButton.data("record")!==app.getRecordId()&&(url+="&crmModule="+app.getModuleName()+"&crmRecord="+app.getRecordId()),thisInstance.sendMailWindow(url,popup);});});},sendMailWindow:function sendMailWindow(url,popup,postData){if(popup){var width=screen.width-15,height=screen.height-150;if(null==postData)return void window.open(url,"_blank","width="+width+", height="+height+", left="+0+", top="+30+",resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,status=no,menubar=no");var form=$("<form/>",{action:"index.php"});for(var i in url.replace(/[?&]+([^=&]+)=([^&]*)/gi,function(m,key,value){form.append($("<input>",{name:key,value:value}));}),postData)form.append($("<input>",{name:i,value:JSON.stringify(postData[i])}));$("body").append(form),form.trigger("submit");}else window.location.href=url;},registerWidgetsEvents:function registerWidgetsEvents(){var widgets=$("div.widgetContainer");widgets.on("shown.bs.collapse",function(e){var widgetContainer=$(e.currentTarget);Vtiger_Index_Js.loadWidgets(widgetContainer);var key=widgetContainer.attr("id");app.cacheSet(key,1);}),widgets.on("hidden.bs.collapse",function(e){var widgetContainer=$(e.currentTarget),imageEle=widgetContainer.parent().find(".imageElement"),imagePath=imageEle.data("rightimage");imageEle.attr("src",imagePath);var key=widgetContainer.attr("id");app.cacheSet(key,0);});},/**
	 ...
	 */loadWidgets:function loadWidgets(widgetContainer,open){var message=$(".loadingWidgetMsg").html();if(widgetContainer.find(".card-body").html().trim()){var imageEle=widgetContainer.parent().find(".imageElement"),imagePath=imageEle.data("downimage");return imageEle.attr("src",imagePath),void widgetContainer.css("height","auto")}widgetContainer.progressIndicator({message:message});var url=widgetContainer.data("url");AppConnector.request({type:"GET",url:"index.php",dataType:"html",data:url}).done(function(data){if("undefined"==typeof open&&(open=!0),open){widgetContainer.progressIndicator({mode:"hide"});var _imageEle=widgetContainer.parent().find(".imageElement"),_imagePath=_imageEle.data("downimage");_imageEle.attr("src",_imagePath),widgetContainer.css("height","auto");}if(widgetContainer.html(data),""==data)widgetContainer.closest(".quickWidget").addClass("d-none");else widgetContainer.closest(".quickWidget").find(".quickWidgetHeader").data("label");$(".bodyContents").trigger("Vtiger.Widget.Load."+label,$(widgetContainer));});},loadWidgetsOnLoad:function loadWidgetsOnLoad(){var widgets=$("div.widgetContainer");widgets.each(function(index,element){Vtiger_Index_Js.loadWidgets($(element));});},/**

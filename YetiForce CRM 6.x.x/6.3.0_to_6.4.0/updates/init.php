...
class YetiForceUpdate
{
    ...
    private function roundcubeUpdateTable()
    {
        ...
        if ($column) {
            $db->createCommand('ALTER TABLE `roundcube_cache` CHANGE `expires` `expires` datetime   NULL after `cache_key`;')->execute();
            $db->createCommand('ALTER TABLE `roundcube_cache` DROP COLUMN `created`;')->execute();
            $db->createCommand('ALTER TABLE `roundcube_cache` ADD PRIMARY KEY(`user_id`,`cache_key`), ROW_FORMAT=DYNAMIC;')->execute();
			...
			CHANGE `cache_key` `cache_key` varchar(128) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL after `user_id`;')->execute();

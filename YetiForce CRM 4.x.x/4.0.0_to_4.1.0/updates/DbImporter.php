...
class DbImporter
{
	...
	/**
	 * Creating tables
	 * @param DbType $importer
	 */
	public function addTables(DbType $importer)
	{
	 ...
	 * @return string
	 */
	public function getOptions(DbType $importer, $table)
	{
		$options = null;
	 ...
	 * @return array
	 */
	public function getColumns(DbType $importer, $table)
	{
	 ...
	 * @return array
	 */
	public function getIndexes(DbType $importer, $table)
	{
	 ...
	 * @param DbType $importer
	 */
	public function addForeignKey(DbType $importer)
	{
	 ...
	 * @param DbType $importer
	 */
	public function addData(DbType $importer)
	{
	 ...
	 * @param DbType $importer
	 */
	public function updateTables(DbType $importer)
	{
		$this->logs .= "> start update tables\n";
		$importer->db->createCommand()->checkIntegrity(false)->execute();
		...
		foreach ($importer->tables as $tableName => $table) {
			...
			if ($indexes = $this->getIndexes($importer, $table)) {
				$dbIndexes = $this->getTableKeys($tableName, $importer);
				foreach ($indexes as $index) {
					try {
			...
			if (isset($table['primaryKeys'])) {
				$dbPrimaryKeys = $this->getPrimaryKey($tableName, $importer);
				foreach ($table['primaryKeys'] as $primaryKey) {
					$status = false;
		...
		$importer->db->createCommand()->checkIntegrity(true)->execute();
	 ...
	 * @param DbType $importer
	 */
	public function updateForeignKey(DbType $importer)
	{
		if (!isset($importer->foreignKey)) {
			return;
		}
	...
	public function getTableKeys($tableName, $importer)
	{
		if (App\Cache::has('getTableKeys', $tableName)) {
			return App\Cache::get('getTableKeys', $tableName);
		}
		$tableName = $importer->db->quoteTableName(str_replace('#__', $importer->db->tablePrefix, $tableName));
		$keys = [];
		try {
			switch ($importer->db->getDriverName()) {
				case 'mysql':
					$dataReader = $importer->db->createCommand()->setSql('SHOW KEYS FROM ' . $tableName)->query();
		...
		App\Cache::save('getTableKeys', $tableName, $keys, App\Cache::LONG);
		return $keys;
	}
	 ...
	 * @return type
	 */
	public function getPrimaryKey($tableName, $importer)
	{
		if (App\Cache::has('getPrimaryKey', $tableName)) {
			return App\Cache::get('getPrimaryKey', $tableName);
		}
		$tableName = $importer->db->quoteTableName(str_replace('#__', $importer->db->tablePrefix, $tableName));
		$key = [];
		switch ($importer->db->getDriverName()) {
			case 'mysql':
				$tableKeys = $this->getTableKeys($tableName, $importer);
				$key = isset($tableKeys['PRIMARY']) ? ['PRIMARY' => array_keys($tableKeys['PRIMARY'])] : [];
				break;
		}
		App\Cache::save('getPrimaryKey', $tableName, $key, App\Cache::LONG);
		return $key;
	}
}

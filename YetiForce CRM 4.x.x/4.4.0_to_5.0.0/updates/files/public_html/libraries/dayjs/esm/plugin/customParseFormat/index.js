var formattingTokens = /(\[[^[]*\])|([-:/.()\s]+)|(A|a|YYYY|YY?|MM?|DD?|hh?|HH?|mm?|ss?|S{1,3}|z|ZZ?)/g;
var match1 = /\d/; // 0 - 9
...
var parseFormattedInput = function parseFormattedInput(input, format) {
  try {
    ...
    var now = new Date();
    return new Date(year || now.getFullYear(), month - 1 || now.getMonth(), day || now.getDate(), hours || 0, minutes || 0, seconds || 0, milliseconds || 0);
  } catch (e) {

	...
	foreach ($data as $message_id => $message) {
		if (isset($message['body'])) {
			if ($message_id) {
				...
				if (
					isset($message['meta']['body_cache'], $message['meta']['body_cache_serial'])
					 &&
...
{
	$phorum_sig = '';
	if (isset($row['user']['signature'], $row['meta']['show_signature'])
		 && $row['meta']['show_signature']==1) {
...
function phorum_htmlpurifier_editor_after_subject()
{
	...
	sort($allowed);
	$allowed_text = implode(', ', $allowed); ?><p><strong>Allowed tags:</strong> <?php
			echo $allowed_text; ?>.</p><?php
		?>
    </p>
    <p>

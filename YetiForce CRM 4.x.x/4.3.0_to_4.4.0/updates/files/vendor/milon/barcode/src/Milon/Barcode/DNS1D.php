...
class DNS1D
{
	 ...
	 * @protected
	 */
	protected function getBarcodePNG($code, $type, $w = 2, $h = 30, $color = [0, 0, 0])
	{
		if (!$this->store_path) {
			$this->setStorPath(app('config')->get('barcode.store_path'));
		}
	 ...
	 * @protected
	 */
	protected function getBarcodePNGPath($code, $type, $w = 2, $h = 30, $color = [0, 0, 0])
	{
		if (!$this->store_path) {
			$this->setStorPath(app('config')->get('barcode.store_path'));
		}
	 ...
	 * @protected
	 */
	protected function getBarcodePNGUri($code, $type, $w = 2, $h = 30, $color = [0, 0, 0])
	{
	...
	protected function barcode_eanupc($code, $len = 13)
	{
		...
		if ($code_len == $data_len) {
			...
			throw new \Milon\Barcode\WrongCheckDigitException($r, (int) ($code[$data_len]));
		}
		if ($len == 12) {
			// UPC-A
			$code = '0' . $code;

...
<d:propfind xmlns:d="DAV:">
  <d:prop>
    <d:supported-report-set />
		...
		$this->assertSame(207, $this->response->status, 'We expected a multi-status response. Full response body: ' . $this->response->body);
		...
		$this->assertSame(1, count($data), 'We expected 1 \'d:prop\' element');

		$data = $xml->xpath('/d:multistatus/d:response/d:propstat/d:prop/d:supported-report-set');
		$this->assertSame(1, count($data), 'We expected 1 \'d:supported-report-set\' element');

		$data = $xml->xpath('/d:multistatus/d:response/d:propstat/d:status');
		$this->assertSame(1, count($data), 'We expected 1 \'d:status\' element');

		$this->assertSame('HTTP/1.1 200 OK', (string) $data[0], 'The status for this property should have been 200');
	}
...
<d:propfind xmlns:d="DAV:">
  <d:prop>
    <d:supported-report-set />
		...
		$this->assertSame(1, count($data), 'We expected 1 \'d:anotherreport\' element. Full body: ' . $this->response->body);

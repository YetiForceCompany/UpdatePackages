...
class SpecificationTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testUpdateRange($headerValue, $httpStatus, $endResult, $contentLength = 4)
	{
		...
		$this->assertSame($httpStatus, $this->server->httpResponse->status, 'Incorrect http status received: ' . $this->server->httpResponse->body);
		if (!is_null($endResult)) {
			$this->assertSame($endResult, file_get_contents(SABRE_TEMPDIR . '/foobar.txt'));
		}
	}

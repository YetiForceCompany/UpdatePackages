...
class ExpandPropertiesTest extends \PHPUnit_Framework_TestCase
{
	public function getServer()
	{
		...
		$this->assertSame($plugin, $fakeServer->getPlugin('acl'));
...
<d:expand-property xmlns:d="DAV:">
  ...
  <d:property name="href" namespace="http://sabredav.org/ns" />
		...
		$this->assertSame(207, $server->httpResponse->status, 'Incorrect status code received. Full body: ' . $server->httpResponse->body);
		$this->assertSame([
		...
		foreach ($check as $v1 => $v2) {
			...
			$this->assertSame($count, count($result), 'we expected ' . $count . ' appearances of ' . $xpath . ' . We found ' . count($result) . '. Full response: ' . $server->httpResponse->body);
		}
	}
...
<d:expand-property xmlns:d="DAV:">
  <d:property name="href" namespace="http://sabredav.org/ns">
      <d:property name="displayname" />
		...
		$this->assertSame(207, $server->httpResponse->status, 'Incorrect response status received. Full response body: ' . $server->httpResponse->body);
		$this->assertSame([
			'X-Sabre-Version' => [DAV\Version::VERSION],
			'Content-Type'    => ['application/xml; charset=utf-8'],
		], $server->httpResponse->getHeaders());
		...
		foreach ($check as $v1 => $v2) {
			...
			$this->assertSame($count, count($result), 'we expected ' . $count . ' appearances of ' . $xpath . ' . We found ' . count($result) . ' Full response body: ' . $server->httpResponse->getBodyAsString());
		}
	}
...
<d:expand-property xmlns:d="DAV:">
  <d:property name="hreflist" namespace="http://sabredav.org/ns">
      <d:property name="displayname" />
		...
		$this->assertSame(207, $server->httpResponse->status);
		$this->assertSame([
			'X-Sabre-Version' => [DAV\Version::VERSION],
			'Content-Type'    => ['application/xml; charset=utf-8'],
		], $server->httpResponse->getHeaders());
		...
		foreach ($check as $v1 => $v2) {
			...
			$this->assertSame($count, count($result), 'we expected ' . $count . ' appearances of ' . $xpath . ' . We found ' . count($result));
		}
	}

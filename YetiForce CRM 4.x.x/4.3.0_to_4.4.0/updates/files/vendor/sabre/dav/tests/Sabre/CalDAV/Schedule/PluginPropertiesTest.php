...
class PluginPropertiesTest extends \Sabre\DAVServerTest
{
	...
	public function testPrincipalProperties()
	{
		...
		$this->assertSame('calendars/user1/outbox/', $prop->getHref());
		...
		$this->assertTrue($prop instanceof DAV\Xml\Property\Href);
		$this->assertSame('calendars/user1/inbox/', $prop->getHref());

		$this->assertArrayHasKey('{urn:ietf:params:xml:ns:caldav}calendar-user-address-set', $props[0][200]);
		$prop = $props[0][200]['{urn:ietf:params:xml:ns:caldav}calendar-user-address-set'];
		$this->assertTrue($prop instanceof DAV\Xml\Property\Href);
		$this->assertSame(['mailto:user1.sabredav@sabredav.org', '/principals/user1/'], $prop->getHrefs());
		...
		$this->assertSame('INDIVIDUAL', $prop);
		...
		$this->assertSame('calendars/user1/default/', $prop->getHref());
	}
	...
	public function testPrincipalPropertiesBadPrincipal()
	{
		...
		$prop = $props[0][200]['{urn:ietf:params:xml:ns:caldav}calendar-user-address-set'];
		$this->assertTrue($prop instanceof DAV\Xml\Property\Href);
		$this->assertSame(['/principals/user1/calendar-proxy-read/'], $prop->getHrefs());

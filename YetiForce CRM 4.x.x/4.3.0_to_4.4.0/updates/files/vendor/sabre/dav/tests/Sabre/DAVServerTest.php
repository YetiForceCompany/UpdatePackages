...
abstract class DAVServerTest extends \PHPUnit_Framework_TestCase
{
	...
	public function request($request, $expectedStatus = null)
	{
		...
		if ($expectedStatus) {
			...
			$this->assertSame($expectedStatus, $response->getStatus(), 'Incorrect HTTP status received for request. Response body: ' . $responseBody);
		}
	...
	public function assertHttpStatus($expectedStatus, HTTP\Request $req)
	{
		...
		$this->assertSame((int) $expectedStatus, (int) $resp->status, 'Incorrect HTTP status received: ' . $resp->body);
	}
}

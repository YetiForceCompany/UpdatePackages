...
ICS;
		...
		$this->assertSame(201, $response->status, 'Incorrect status returned! Full response body: ' . $response->body);
		$this->assertSame([
		...
		$this->assertSame($expected, $this->calBackend->getCalendarObject('calendar1', 'blabla.ics'));
	}
...
ICS;
		...
		$this->assertSame(415, $response->status, 'Incorrect status returned! Full response body: ' . $response->body);
	}
...
ICS;
		...
		$response = $this->request($request);
		$this->assertSame(403, $response->status, 'Incorrect status returned! Full response body: ' . $response->body);
	}

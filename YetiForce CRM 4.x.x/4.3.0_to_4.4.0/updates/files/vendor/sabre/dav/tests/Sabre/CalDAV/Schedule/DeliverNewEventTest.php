...
ICS
		...
		$this->assertSame(201, $response->getStatus(), 'Incorrect status code received. Response body:' . $response->getBodyAsString());
		...
		$this->assertSame('mailto:user2.sabredav@sabredav.org', $message->recipient);
		$this->assertSame('Roxy Kesh', $message->recipientName);
		$this->assertSame('mailto:user1.sabredav@sabredav.org', $message->sender);
		$this->assertSame('Administrator', $message->senderName);
		$this->assertSame('REQUEST', $message->method);

		$this->assertSame('REQUEST', $message->message->METHOD->getValue());
	}
}

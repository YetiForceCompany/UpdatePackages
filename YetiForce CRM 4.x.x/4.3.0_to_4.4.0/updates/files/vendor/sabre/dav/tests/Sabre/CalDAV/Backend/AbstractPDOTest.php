...
abstract class AbstractPDOTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testCreateCalendarAndFetch()
	{
		...
		$this->assertSame(1, count($calendars));

		foreach ($elementCheck as $name => $value) {
			$this->assertArrayHasKey($name, $calendars[0]);
			$this->assertSame($value, $calendars[0][$name]);
		}
	}
	...
	public function testGetCalendarObjects()
	{
		...
		$this->assertSame('random-id', $data['uri']);
		$this->assertSame(strlen($object), $data['size']);
	}
	...
	public function testUpdateCalendarObject()
	{
		...
		$this->assertSame($object2, $data['calendardata']);
		$this->assertSame('random-id', $data['uri']);
	}
	...
	public function testCreateSubscriptions()
	{
		...
		$this->assertSame(1, count($subs));
		foreach ($expected as $k => $v) {
			$this->assertSame($subs[0][$k], $expected[$k]);
		}
	}
	...
	public function testUpdateInvitesNoPrincipal()
	{
		...
		$this->assertSame($expected, $result, null, 0.0, 10, true); // Last argument is $canonicalize = true, which allows us to compare, ignoring the order, because it's different between MySQL and Sqlite.
	}

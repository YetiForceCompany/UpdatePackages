...
class MSWordTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testLockEtc()
	{
		...
		$this->assertSame(201, $server->httpResponse->getStatus(), 'Full response body:' . $response1->getBodyAsString());
		...
		$this->assertSame(201, $server->httpResponse->status);
		$this->assertTrue((bool) $server->httpResponse->getHeaders('Lock-Token'));
		...
		$this->assertSame(204, $server->httpResponse->status);
	}

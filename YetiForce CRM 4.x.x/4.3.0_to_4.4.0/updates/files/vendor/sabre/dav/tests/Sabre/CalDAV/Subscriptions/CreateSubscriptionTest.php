...
XML;
	...
	{
		$this->assertSame('', $subscription['{http://calendarserver.org/ns/}subscribed-strip-attachments']);
		$this->assertSame('', $subscription['{http://calendarserver.org/ns/}subscribed-strip-todos']);
		$this->assertSame('#1C4587FF', $subscription['{http://apple.com/ns/ical/}calendar-color']);
		$this->assertSame('Jewish holidays', $subscription['{DAV:}displayname']);
		$this->assertSame('Foo', $subscription['{urn:ietf:params:xml:ns:caldav}calendar-description']);
		$this->assertSame('19', $subscription['{http://apple.com/ns/ical/}calendar-order']);
		$this->assertSame('webcal://www.example.org/', $subscription['{http://calendarserver.org/ns/}source']->getHref());
		$this->assertSame('P1W', $subscription['{http://apple.com/ns/ical/}refreshrate']);
		$this->assertSame('subscription1', $subscription['uri']);
		$this->assertSame('principals/user1', $subscription['principaluri']);
		$this->assertSame('webcal://www.example.org/', $subscription['source']);
		$this->assertSame(['principals/user1', 1], $subscription['id']);
	}
}

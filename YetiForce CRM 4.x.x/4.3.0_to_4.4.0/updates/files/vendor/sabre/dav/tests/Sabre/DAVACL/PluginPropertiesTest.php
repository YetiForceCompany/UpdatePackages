...
class PluginPropertiesTest extends \PHPUnit_Framework_TestCase
{
	public function testPrincipalCollectionSet()
	{
		...
		$this->assertSame(1, count($result[200]));
		...
		$this->assertSame($expected, $result[200]['{DAV:}principal-collection-set']->getHrefs());
	}
	...
	public function testCurrentUserPrincipal()
	{
		...
		$this->assertSame(Xml\Property\Principal::UNAUTHENTICATED, $result[200]['{DAV:}current-user-principal']->getType());
		...
		$this->assertArrayHasKey('{DAV:}current-user-principal', $result[200]);
		$this->assertInstanceOf('Sabre\DAVACL\Xml\Property\Principal', $result[200]['{DAV:}current-user-principal']);
		$this->assertSame(Xml\Property\Principal::HREF, $result[200]['{DAV:}current-user-principal']->getType());
		$this->assertSame('principals/admin/', $result[200]['{DAV:}current-user-principal']->getHref());
	}
	...
	public function testSupportedPrivilegeSet()
	{
		...
		foreach ($xpaths as $xpath => $count) {
			$this->assertSame($count, $dxpath->query($xpath)->length, 'Looking for : ' . $xpath . ', we could only find ' . $dxpath->query($xpath)->length . ' elements, while we expected ' . $count . ' Full XML: ' . $result);
		}
	}
	...
	public function testACL()
	{
		...
		$this->assertSame(1, count($result[200]), 'The {DAV:}acl property did not return from the list. Full list: ' . print_r($result, true));
	...
	public function testACLRestrictions()
	{
		...
		$this->assertSame(1, count($result[200]), 'The {DAV:}acl-restrictions property did not return from the list. Full list: ' . print_r($result, true));
	...
	public function testAlternateUriSet()
	{
		...
		$this->assertSame([], $result[200]['{DAV:}alternate-URI-set']->getHrefs());
	}
	...
	public function testPrincipalURL()
	{
		...
		$this->assertSame('principals/user/', $result[200]['{DAV:}principal-URL']->getHref());
	}
	...
	public function testGroupMemberSet()
	{
		...
		$this->assertSame([], $result[200]['{DAV:}group-member-set']->getHrefs());
	}
	...
	public function testGroupMemberShip()
	{
		...
		$this->assertSame([], $result[200]['{DAV:}group-membership']->getHrefs());
	}
	...
	public function testGetDisplayName()
	{
		...
		$this->assertSame('user', $result[200]['{DAV:}displayname']);
	}
}

...
BLA;
		...
		$this->assertSame(207, $response->status, 'Full response body:' . $response->body);
		...
		$this->assertSame(2, count($responses), 'We expected exactly 2 {DAV:}response');

		$response = $responses[0];

		$this->assertNull($response->getHttpStatus());
		$this->assertSame('/coll/file1.txt', $response->getHref());
		$this->assertSame([
			200 => [
		...
		$this->assertNull($response->getHttpStatus());
		$this->assertSame('/coll/file2.txt', $response->getHref());
		$this->assertSame([
			200 => [
				'{DAV:}getcontentlength' => 3,
			]
...
BLA;
		...
		$this->assertSame('/coll/file3.txt', $response->getHref());
		$this->assertSame([], $response->getResponseProperties());
	}
...
BLA;
		...
		$responses = $multiStatus->getResponses();
		$this->assertSame(1, count($responses), 'We expected exactly 1 {DAV:}response');
...
BLA;
		...
		// The default state has no sync-token, so this report should not yet
		// be supported.
		$this->assertSame(403, $response->status, 'Full response body:' . $response->body);
	}

...
class PrincipalTest extends \PHPUnit_Framework_TestCase
{
	public function testSimple()
	{
		...
		$this->assertSame(Principal::UNAUTHENTICATED, $principal->getType());
		$this->assertNull($principal->getHref());

		$principal = new Principal(Principal::AUTHENTICATED);
		$this->assertSame(Principal::AUTHENTICATED, $principal->getType());
		$this->assertNull($principal->getHref());

		$principal = new Principal(Principal::HREF, 'admin');
		$this->assertSame(Principal::HREF, $principal->getType());
		$this->assertSame('admin/', $principal->getHref());
	}
...
'</d:principal>';
		...
		$this->assertSame(Principal::HREF, $principal->getType());
		$this->assertSame('/principals/admin/', $principal->getHref());
	}

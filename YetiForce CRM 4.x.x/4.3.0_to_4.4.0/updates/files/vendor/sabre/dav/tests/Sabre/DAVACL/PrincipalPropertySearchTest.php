...
<d:principal-property-search xmlns:d="DAV:">
  ...
  <d:prop>
    ...
    <d:getcontentlength />
		...
		$this->assertSame(400, $server->httpResponse->getStatus(), $server->httpResponse->getBodyAsString());
		$this->assertSame([
...
<d:principal-property-search xmlns:d="DAV:">
  ...
  <d:prop>
    ...
    <d:getcontentlength />
		...
		$this->assertSame(207, $server->httpResponse->getStatus(), 'Full body: ' . $server->httpResponse->getBodyAsString());
		$this->assertSame([
			'X-Sabre-Version' => [DAV\Version::VERSION],
			'Content-Type'    => ['application/xml; charset=utf-8'],
			'Vary'            => ['Brief,Prefer'],
...
<d:principal-property-search xmlns:d="DAV:">
  ...
  <d:prop>
    ...
    <d:getcontentlength />
		...
		$this->assertSame(207, $server->httpResponse->status, $server->httpResponse->body);
		$this->assertSame([
			'X-Sabre-Version' => [DAV\Version::VERSION],
			'Content-Type'    => ['application/xml; charset=utf-8'],
			'Vary'            => ['Brief,Prefer'],
		...
		foreach ($check as $v1 => $v2) {
			...
			$this->assertSame($count, count($result), 'we expected ' . $count . ' appearances of ' . $xpath . ' . We found ' . count($result) . '. Full response body: ' . $server->httpResponse->body);
		}
	}

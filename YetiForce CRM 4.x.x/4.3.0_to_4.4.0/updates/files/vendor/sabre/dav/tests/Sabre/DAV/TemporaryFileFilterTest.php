...
class TemporaryFileFilterTest extends AbstractServer
{
	...
	public function testPutNormal()
	{
		...
		$this->assertSame('0', $this->response->getHeader('Content-Length'));

		$this->assertSame('Testing new file', file_get_contents(SABRE_TEMPDIR . '/testput.txt'));
	}
	...
	public function testPutGet()
	{
		...
		$this->assertSame('Testing new file', stream_get_contents($this->response->body));
	}
...
<D:lockinfo xmlns:D="DAV:">
    ...
    <D:owner>
		...
		$this->assertSame('true', $this->response->getHeader('X-Sabre-Temp'));
	...
	public function testPutDelete()
	{
		...
		$this->assertSame(204, $this->response->status, "Incorrect status code received. Full body:\n" . $this->response->body);
		$this->assertSame([
			'X-Sabre-Temp' => ['true'],
		], $this->response->getHeaders());
	...
	public function testPutPropfind()
	{
		...
		$this->assertSame(207, $this->response->status, 'Incorrect status code returned. Body: ' . $this->response->body);
		$this->assertSame([
			'X-Sabre-Temp' => ['true'],
			'Content-Type' => ['application/xml; charset=utf-8'],
		], $this->response->getHeaders());
		...
		$this->assertSame('/._testput.txt', (string) $data, 'href element should have been /._testput.txt');

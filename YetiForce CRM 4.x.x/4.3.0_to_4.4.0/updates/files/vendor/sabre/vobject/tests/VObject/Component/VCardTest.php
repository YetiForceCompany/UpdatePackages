...
class VCardTest extends TestCase
{
	...
	public function testValidate($input, $expectedWarnings, $expectedRepairedOutput)
	{
		...
		$this->assertSame($expectedWarnings, $warnMsg);
	...
	public function testGetDocumentType()
	{
		...
		$vcard->VERSION = '2.1';
		$this->assertSame(VCard::VCARD21, $vcard->getDocumentType());

		$vcard = new VCard([], false);
		$vcard->VERSION = '3.0';
		$this->assertSame(VCard::VCARD30, $vcard->getDocumentType());

		$vcard = new VCard([], false);
		$vcard->VERSION = '4.0';
		$this->assertSame(VCard::VCARD40, $vcard->getDocumentType());

		$vcard = new VCard([], false);
		$this->assertSame(VCard::UNKNOWN, $vcard->getDocumentType());
	}
...
VCF;
		...
		$this->assertSame('1@example.org', $vcard->getByType('EMAIL', 'home')->getValue());
		$this->assertSame('2@example.org', $vcard->getByType('EMAIL', 'work')->getValue());
...
VCF;
		...
		$vcard = VObject\Reader::read($vcard);
		$this->assertSame('1@example.org', $vcard->preferred('EMAIL')->getValue());
	}
...
VCF;
		...
		$vcard = VObject\Reader::read($vcard);
		$this->assertSame('2@example.org', $vcard->preferred('EMAIL')->getValue());
	}
...
VCF;
		...
		$vcard = VObject\Reader::read($vcard);
		$this->assertSame('3@example.org', $vcard->preferred('EMAIL')->getValue());
	}
...
VCF;
	...
	public function assertValidateResult($input, $expectedLevel, $expectedMessage = null)
	{
		...
		if ($expectedLevel === 0) {
			$this->assertSame(0, count($input), 'No validation messages were expected. We got: ' . implode(', ', $messages));
		} else {
			$this->assertSame(1, count($input), 'We expected exactly 1 validation message, We got: ' . implode(', ', $messages));

			$this->assertSame($expectedMessage, $input[0]['message']);
			$this->assertSame($expectedLevel, $input[0]['level']);
		}
	}
}

...
';
	...
	public function testGetDocumentType()
	{
		$vcard = new VCalendar();
		$vcard->VERSION = '2.0';
		$this->assertSame(VCalendar::ICALENDAR20, $vcard->getDocumentType());
	}
...
';
		...
		$vcal = VObject\Reader::read($input);
		$this->assertSame([], $vcal->validate(), 'Got an error');
	}
...
';
		...
		$vcal = VObject\Reader::read($input);
		$this->assertSame(1, count($vcal->validate()));
	}
...
';
		...
		$vcal = VObject\Reader::read($input);
		$this->assertSame(0, count($vcal->validate()));
	}
...
';
		...
		$this->assertSame('test', $result->SUMMARY->getValue());
	}

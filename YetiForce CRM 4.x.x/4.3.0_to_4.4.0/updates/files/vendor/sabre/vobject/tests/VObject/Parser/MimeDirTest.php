...
VCF;
		...
		$this->assertSame("umlaut u - \xC3\xBC", $vcard->FN->getValue());
	}
...
VCF;
		...
		$mimeDir = new MimeDir();
		$vcard = $mimeDir->parse($vcard);
		$this->assertSame("foo-bar - \xFC", $vcard->FN->getValue());
	}
...
VCF;
		...
		$this->assertSame("umlaut u - \xFC", $vcard->FN->getValue());
	}
...
VCF;
		...
		$vcard = $mimeDir->parse($vcard);
		$this->assertSame("Euro \xE2\x82\xAC", $vcard->FN->getValue());
	}
...
VCF;
		...
		$this->assertSame('Euro', $vcard->FN->getValue());
		$this->assertSame('Test2', $vcard->N->getValue());
	}
}

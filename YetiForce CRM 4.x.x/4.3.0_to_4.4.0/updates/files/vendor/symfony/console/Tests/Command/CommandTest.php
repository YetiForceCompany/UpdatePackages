...
class CommandTest extends TestCase
{
	...
	public function testConstructor()
	{
		...
		$this->assertSame('foo:bar', $command->getName(), '__construct() takes the command name as its first argument');
	}
	...
	public function testSetApplication()
	{
		...
		$this->assertSame($application, $command->getApplication(), '->setApplication() sets the current application');
		$this->assertSame($application->getHelperSet(), $command->getHelperSet());
	}
	...
	public function testSetGetDefinition()
	{
		...
		$this->assertSame($command, $ret, '->setDefinition() implements a fluent interface');
		$this->assertSame($definition, $command->getDefinition(), '->setDefinition() sets the current InputDefinition instance');
	...
	public function testAddArgument()
	{
		...
		$this->assertSame($command, $ret, '->addArgument() implements a fluent interface');
	...
	public function testAddOption()
	{
		...
		$this->assertSame($command, $ret, '->addOption() implements a fluent interface');
	...
	{
		$command = new \TestCommand();
		$this->assertSame('namespace:name', $command->getName(), '->getName() returns the command name');
		$command->setName('foo');
		$this->assertSame('foo', $command->getName(), '->setName() sets the command name');

		$ret = $command->setName('foobar:bar');
		$this->assertSame($command, $ret, '->setName() implements a fluent interface');
		$this->assertSame('foobar:bar', $command->getName(), '->setName() sets the command name');
	}
	...
	{
		$command = new \TestCommand();
		$this->assertSame('description', $command->getDescription(), '->getDescription() returns the description');
		$ret = $command->setDescription('description1');
		$this->assertSame($command, $ret, '->setDescription() implements a fluent interface');
		$this->assertSame('description1', $command->getDescription(), '->setDescription() sets the description');
	}

	public function testGetSetHelp()
	{
		$command = new \TestCommand();
		$this->assertSame('help', $command->getHelp(), '->getHelp() returns the help');
		$ret = $command->setHelp('help1');
		$this->assertSame($command, $ret, '->setHelp() implements a fluent interface');
		$this->assertSame('help1', $command->getHelp(), '->setHelp() sets the help');
		$command->setHelp('');
		$this->assertSame('', $command->getHelp(), '->getHelp() does not fall back to the description');
	}
	...
	{
		$command = new \TestCommand();
		$this->assertSame(['name'], $command->getAliases(), '->getAliases() returns the aliases');
		$ret = $command->setAliases(['name1']);
		$this->assertSame($command, $ret, '->setAliases() implements a fluent interface');
		$this->assertSame(['name1'], $command->getAliases(), '->setAliases() sets the aliases');
	}
	...
	public function testGetSynopsis()
	{
		...
		$this->assertSame('namespace:name [--foo] [--] [<bar>]', $command->getSynopsis(), '->getSynopsis() returns the synopsis');
	}
	...
	public function testGetHelper()
	{
		...
		$this->assertSame($formatterHelper->getName(), $command->getHelper('formatter')->getName(), '->getHelper() returns the correct helper');
	}
	...
	public function testMergeApplicationDefinition()
	{
		...
		$m->invoke($command);
		$this->assertSame(3, $command->getDefinition()->getArgumentCount(), '->mergeApplicationDefinition() does not try to merge twice the application arguments and options');
	}
	...
	public function testMergeApplicationDefinitionWithoutArgsThenWithArgsAddsArgs()
	{
		...
		$m->invoke($command);
		$this->assertSame(2, $command->getDefinition()->getArgumentCount(), '->mergeApplicationDefinition() does not try to merge twice the application arguments');
	}
	...
	public function testRunInteractive()
	{
		...
		$this->assertSame('interact called' . PHP_EOL . 'execute called' . PHP_EOL, $tester->getDisplay(), '->run() calls the interact() method if the input is interactive');
	}
	...
	public function testRunNonInteractive()
	{
		...
		$this->assertSame('execute called' . PHP_EOL, $tester->getDisplay(), '->run() does not call the interact() method if the input is not interactive');
	}
	...
	public function testRunWithProcessTitle()
	{
		...
		if (function_exists('cli_set_process_title')) {
			...
			$this->assertSame('foo', cli_get_process_title());
		}
	}
	...
	public function testSetCode()
	{
		...
		$this->assertSame($command, $ret, '->setCode() implements a fluent interface');
		$tester = new CommandTester($command);
		$tester->execute([]);
		$this->assertSame('interact called' . PHP_EOL . 'from the code...' . PHP_EOL, $tester->getDisplay());
	}
	...
	public function testSetCodeBindToClosure($previouslyBound, $expected)
	{
		...
		$tester = new CommandTester($command);
		$tester->execute([]);
		$this->assertSame('interact called' . PHP_EOL . $expected . PHP_EOL, $tester->getDisplay());
	}
	...
	public function testSetCodeWithStaticClosure()
	{
		...
		$this->assertSame('interact called' . PHP_EOL . 'bound' . PHP_EOL, $tester->getDisplay());
	}
